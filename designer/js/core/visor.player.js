(function($) {
	player=function(opt){
		...
		this.mousedown=function(e){
			...
			var ths = this;
			this.mousemove = function(e){
//				console.log(e.desktopX+','+e.desktopY+';'+e.originalEvent.clientX+','+e.originalEvent.clientY);
                
//              ths.activePanel.canvas.offsetLeft = e.originalEvent.clientX;
	...
	player.prototype.init=function(doc){
		...
		if(id!==undefined){	
			...
			if(currentdoc===null){
				...
				var contentpar = JSON.parse(currentdoc.content);			
				for(var i = contentpar.panels.length-1; i >= 0; i--){
					contentpar.panels[i].showgrid = false;
				}
				var contentstr = JSON.stringify(contentpar);
				$.extend(currentdoc, {content: contentstr});
			}
			 if(currentdoc&&currentdoc!="undefined"){

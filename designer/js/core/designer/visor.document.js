	...
	visordocument=function(parent,opt){
		...
		this.restoreSchema=function(data){
			...
			for(var i=0;i<=data.schema.tables.length-1;i++){
				...
				x=x+20;
				y=y+20;
				table1.fieldclickEvent=designer.fieldclickEvent;
...
//				opt.click=designer.widgetClickEvent;
				...
				widgetName=this.activePanel.instance.getName(widgetType);		
				opt={
					name:widgetName,
					text:widgetName,
					focus:true,
	...
	visorpanel=function(parent,opt,document){
		...
		if(!_dom){
			_dom=$("<div></div>").addClass("designer")
			 .attr("id",_name).css("left",offsetx).css("top",offsety).css("position","relative");
...
//	   			$.command("new",instance,newObj);
		...
		for(var i=0;i<opt.widgets.length;i++){
		 ...
		 instance.adddefaultconnection=function(e){
			var targetwidget;
			if(this.targetwidget.type=="objectTree"&&this.targetwidget.widgets.length>0){
        	...
        	if(this.focuswidget.type==="table"&&this.targetwidget.type==="objectTree")
			 ...
			 $(instance.canvas).contextmenu({
				 'target':'#context-menu',
				  ...
				  before: function (e, element, target) {
				      ...
				      var _widget=instance.focuswidget;
				      if(_widget==null||((_widget!=null)&&_widget.type!=="entity"&&_widget.type!=="table"&&_widget.type!=="objectTree"&&_widget.type!=="treeNode")){
    ...
    visorpanel.prototype.listMapping=function(){
    	...
    	for(var i=0;i<=widgets.length-1;i++){
    		if(widgets[i].type=="mapConnector"){
    			...
    			if(mapping.relationship=="ManyOne"){
    				mapping.target_path=paths[paths.length-1];
    			}

...
//			console.info(this.widgets[i].name+"-"+this.widgets[i].height);
	...
	math = {
			...
			isPointOnLine:function(px0, py0, px1,py1,px2,py2){
			        flag = false;  
			        ESP = 250;  
			        if ((Math.abs(math.Multiply(px0, py0, px1, py1, px2, py2)) < ESP)  
			                && ((px0 - px1) * (px0 - px2) <= ESP)  
			                && ((py0 - py1) * (py0 - py2) <= ESP)) {  
			            flag = true;  
			        }  
	...
	propertyObject=function(option){
		var opt= {
			datatype:{
				...
				required:false,
				editable:true,
				options:[{id:'String',text:"String"},{id:'Integer',text:"Integer"},{id:'Date',text:"Date"},{id:'Boolean',text:"Boolean"},{id:'Array',text:"Array"},{id:'Object',text:"Object"}]
			},
			mappingtype:{
				format:"single",
				title:"Mapping Type",
...
(function($) {
	...
	animation = function(option) {
		...
		this.startValues={};
		this.endValues={};	
		this.requiredPaint=false;	
		this.duration=parseFloat(this.duration);
		this.delay=parseFloat(this.delay);
		
		var ths=this;
		this.title=this.widget.name+"-"+this.action;
		this.name=this.name||this.widget.name;
				
		if(typeof tweenEnterType!="undefined"){
			$(tweenEnterType).each(function(i,item){
				if(item.id==ths.enterstyle){
					ths.action=item.name;
					return;
				}
			});
		}

		if(typeof tweenLeaveType!="undefined"){
			$(tweenLeaveType).each(function(i,item){
				if(item.id==ths.leavestyle){
					ths.action=ths.action+","+item.name;
					return;
				}
			});
		}	
		this.init();
		for(var p in this.startValues){
			this.endValues[p]=this.widget[p];
		}	
	};
	
	animation.prototype.init=function(){
		if(this.enterstyle==1){
			this.startValues.x=-this.widget.width-20;
		}
		else if(this.enterstyle==2){
			this.startValues.x=this.widget.parent.width+20;
		}
		else if(this.enterstyle==3){
			this.startValues.y=-this.widget.height;
		}
		else if(this.enterstyle==4){
			this.startValues.y=this.widget.parent.height+20;
		}
		else if(this.enterstyle==5){
			this.startValues.width=0;
			this.startValues.x=this.widget.x+this.widget.width-this.widget.minwidth;			
		}
		else if(this.enterstyle==6){
			this.startValues.height=0;						
		}
		else if(this.enterstyle==7){
			this.startValues.width=0;			
			this.startValues.x=this.widget.x+(this.widget.width-this.widget.minwidth)/2;
		}
		else if(this.enterstyle==8){			
			this.startValues.height=0;			
			this.startValues.y=this.widget.y+this.widget.minheight/2;
		}
		else if(this.enterstyle==9){
			this.startValues.height=0;
			this.startValues.y=this.widget.y+this.widget.height-this.widget.minheight;
		}
		else if(this.enterstyle==10){
			this.startValues.width=0;
		}
		else if(this.enterstyle==11){
			this.startValues.width=0;
			this.startValues.height=0;
		}
		...
		if(this.zoom==1){
			this.startValues.width=this.widget.minwidth;
			this.startValues.height=this.widget.minheight;
		}
		
		
		if(this.fade==1){
			 this.startValues.alpha=0;
		}	
		
		 if(this.rotate>0)
		 	this.startValues.rotate=-this.rotate;
		 else{
		 	this.startValues.rotate=-this.rotate;			 	
		}
	}
	...
	animation.prototype.reset=function(){
		this.init();
		for(var p in this.startValues){
			this.widget[p]=this.startValues[p];
		}	
		this.widget.alpha=0;
		$(this.widget.connections).each(function(i,connection){
	...
	animation.prototype.persist=function(){
		...
		r.enterstyle=this.enterstyle;
		r.leaveStyle=this.leaveStyle;
		r.paused=this.paused;
		r.widget=this.widget.name;
		r.name=this.name;
		r.zoom=this.zoom;
	...
	animation.prototype.enter=function(){
		  ...
		  var postition=this.startValues;
		  this.widget.alpha=1;
	      var tweenA=new TWEEN.Tween(postition).to(this.endValues, this.duration*1000).delay(this.delay*1000).repeat(this.repeat).easing(eval(ths.easing)).onUpdate(function(){	    	  
		      for(var s in ths.startValues){
		      		ths.widget[s]=postition[s];
		      }
	      }).onComplete(function(){
	      	 $(ths.widget.connections).each(function(i,connection){
	      		connection.visible=true; 
	      	 });     	 
	      });	      
	      tweenA.widget=this.widget;
	      tweenA.start();
	      return tweenA;
	};


	animation.prototype.leave=function(){
		  var ths=this;		
		  for(var p in this.startValues){
				this.endValues[p]=this.widget[p];				
			}		  
		  var postition=this.startValues;
      return new TWEEN.Tween(this.endValues).to(postition, this.duration).delay(this.delay).repeat(this.repeat).easing(ths.easing).onUpdate(function(){
      	for(var s in ths.startValues){
      		ths.widget[s]=this[s];
      	}
      }).onComplete(function(){
	...
	(function() {
	    ...
	    for(var x = 0; x < vendors.length && !window.requesttweenFrame; ++x) {
	        window.requestAnimFrame = window[vendors[x]+'RequesttweenFrame'];
	        window.canceltweenFrame = window[vendors[x]+'CanceltweenFrame'] 
	                                   || window[vendors[x]+'CancelRequesttweenFrame'];
	    }
	    ...
	    if (!window.canceltweenFrame)
	        window.canceltweenFrame = function(id) {
	            clearTimeout(id);
	        };
	}());
...
(function($){
	...
	treeNode=function(option){
		...
		else if(this.objectdata.datatype=="Object"||this.objectdata.datatype=="MergedObject")
		...
		this.click=function(e){
			var inputbox=this.connection().presenters[0].textbox;
			for(var i=0;i<=this.parent.widgets.length-1;i++){
				this.parent.widgets[i].focus=false;
			}
			this.focus=true;
			this.connection().presenters[0].focuswidget=this;
			
			$(inputbox).hide();
			this.connection().presenters[0].paint();			
			if(this.connection().fieldclickEvent){
				this.connection().fieldclickEvent.call(this,e);
			}
			e.cancel=true;
		};
		...
		this.connection=function(){
			  ...
			  while(connection&&connection.type!="connection")
		...
		this.persist=function(){			
			var r=widget.persistproperty(this);
			r.treename=this.connection().name;
			r.objectdata={};
		...
		this.deletefield=function(){
			...
			if(index<0)
				return;
			var presenter=this.connection().presenters[0];
		...
		this.beforePaint=function(ctx){
			if(this.parent.type=="connection")
			...
			if(this.widgets.length>0){
				...
				if(this.objectdata.targetpath){
					var connector=this.connection().presenters[0].Widget(this.objectdata.targetpath);
					if(connector){
		...
		this.drawBorderTo=function(ctx){
			...
			if (_widget.border.color !== "none") {
				...
				ctx.strokeRect(left,1.5,_widget.width-left-(this.objectdata.layer+1)*2+0.5,_widget.height-this.margin+2);
			}
			this.drawImageTo(ctx);	
			ctx.restore();
			return this;
		...
		this.drawTextTo=function(ctx){
			...
			if(this.focus){
				ctx.fillStyle= "lightblue";
				var left=this.margin+this.objectdata.layer*(layerinst+iconwidth)+3;
				ctx.fillRect(left,0,this.width-left-(this.objectdata.layer+1)*2,this.height);
				ctx.fillStyle="white";
				ctx.strokeStyle="white";
			}
			else{
		...
		this.keypress=function(e){
			if(e.keyCode===46){  //delete
				...
				if(e.ctrlKey){					
					...
					this.focus=false;
					if(index>0){
						this.connection().presenters[0].focuswidget=parent.widgets[index-1];
						parent.widgets[index-1].focus=true;
					}
					else if(ths.parent.type=="treeNode"){
						this.connection().presenters[0].focuswidget=ths.parent;
	...
	connection=function(option){
		...
		this.beforePaint=function(ctx){
			...
			$(this.widgets).each(function(i,item){
				ths.minwidth=Math.max(ths.minwidth,item.minwidth);
//				item.y=(i+1)*(ths.lineheight)+ths.margin;
	...
	mapConnector=function(option){
		...
		var ths=this;
//		if(!this.objectdata.target&&this.objectdata.targetpath){
//			var paths=this.objectdata.targetpath.split(".");
//			if(paths.length>0){
//				this.end.widget.includeChildren=true;
//				var connection=this.presenters[0].Widget(this.end.widget.name);
//				this.objectdata.target=connection.Widget(paths[0]);
...
//		}
		...
		this.persist=function(){
			...
			if(this.objectdata.target){
				r.objectdata.targetpath=gettargetpath(this.objectdata.target,1);
		...
		this.ondelete=function(){
			...
			if(this.objectdata.target)
				...
				if(paths.length>0){
					this.end.widget.includeChildren=true;
					var connection=this.presenters[0].Widget(this.end.widget.name);
					this.objectdata.target=connection;
					for(var i=1;i<=paths.length-1;i++){
						this.objectdata.target=this.objectdata.target.Widget(paths[i]);
					}
					this.objectdata.target.parent.removeWidget(this.objectdata.target);
		...
		this.onconnected=function(target){
			...
			var src='data:image/png;base64,';

...
(function($){
	...
	widget.prototype.drawResizerTo=function(ctx){
		let optionList=localStorage.getItem('optionList') ? JSON.parse(localStorage.getItem('optionList')) : [];//缓存创建的图形
		ctx.save();
		ctx.beginPath();
		ctx.globalAlpha = 1;
		...
		if(!this.autosize){
			for(var i=0;i<=this.resizers.length-1;i++){
			  if(this.resizers[i].visible)
				 ...
				 if(this.rotate===0){
					ctx.fillText(this.x,-this.x/2,-10)//横线字
					ctx.fillText(this.y,10,-this.y/2)//竖线字
					drawDashLine(ctx,-this.x,0,0,0,2,resizerColor)//横线
					drawDashLine(ctx,0,-this.y,0,0,2,resizerColor)//竖线
				 }else{
					
					// ctx.fillText(updatePoint.x,-updatePoint.x/2,-10)//横线字
				 }

			}
			//旋转后更新下,坐标
			// let centerPoint={x:this.resizers[1].x/2+this.x,y:this.resizers[2].y/2+this.y}//中心点
			// let updatePoint=rotatePoint({x:this.x,y:this.y},centerPoint,this.rotate)//更新后的坐标
			// if(this.rotate==20){
			// 	drawDashLine(ctx,0.0,-updatePoint.x,0,2)//竖线
			// }				
		} 
		// ctx.font = "30px Verdana";
		ctx.restore();
		// this.option 绘制图形的基本信息
		if(this.rotate===0){
			if(optionList.length==0){
				optionList.push({name:this.option.name,xyWH:[
					{x:this.x,y:this.y},
				...
				for(var i=0;i<optionList.length;i++){
					if(this.option.name==optionList[i].name){
						// optionList.splice(0,i)
					}else{
					//存储图形
					optionList.push({name:this.option.name,xyWH:[
						{x:this.x,y:this.y},
					...
					optionList=unique(optionList, "name")
					localStorage.setItem('optionList',JSON.stringify(optionList))
					}
				}
				if(this.option){
					const oList=JSON.parse(localStorage.getItem('optionList'));
					const standard='left';//默认靠左为最近
					let optionInform={};
					for(var i=0;i<oList.length;i++){
						if(this.option.name==oList[i].name){
							console.log(oList[i].square)
							optionInform=oList[i].square
							break;
						}
					}
					//发生碰撞（因为画了线会一直碰撞）
					// for(var i=0;i<oList.length;i++){
					// 	if(optionInform.name===oList[i].name){
					// 		continue
					// 	}else{
					// 		console.log(intersects({x:optionInform.x,y:optionInform.y,width:optionInform.width,height:optionInform.height},oList[0].square))
					// 		if(intersects(oList[i].square,{x:optionInform.x,y:optionInform.y,width:optionInform.width,height:optionInform.height})){
					// 			alert('发生碰撞');
					// 		}
					// 	}
					// }
					...
					for(var i=0;i<oList.length;i++){
						if(this.option.name===oList[i].name){
							...
							//以选中的图像为原点(左上角)连接所有图形圆点
							// drawDashLine(ctx,-(optionInform.x-oList[i].square.x),oList[i].square.y-optionInform.y,0,0,2)
							//竖线
							drawDashLine(ctx,-(optionInform.x-oList[i].square.x),oList[i].square.y-optionInform.y,-(optionInform.x-oList[i].square.x),0,2,colorList[Math.floor(Math.random() * colorList.length)])
							drawDashLine(ctx,-(optionInform.x-oList[i].square.x),0,0,0,2,colorList[Math.floor(Math.random() * colorList.length)])
							ctx.fillText(Math.abs(oList[i].square.y-optionInform.y),-(optionInform.x-oList[i].square.x),oList[i].square.y/2-50)
							ctx.fillStyle=colorList[Math.floor(Math.random() * colorList.length)]
							ctx.fillText(Math.abs(oList[i].square.x-optionInform.x),-(optionInform.x-oList[i].square.x)/2,-10)
							ctx.fillStyle=colorList[Math.floor(Math.random() * colorList.length)]
	...
	//绘制虚线
	function drawDashLine(ctx, x1, y1, x2, y2, dashLength,color=resizerColor){
		ctx.beginPath();
		var dashLen = dashLength === undefined ? 5 : dashLength,
		xpos = x2 - x1, //得到横向的宽度;
		ypos = y2 - y1, //得到纵向的高度;
		numDashes = Math.floor(Math.sqrt(xpos * xpos + ypos * ypos) / dashLen); 
		//正切获取斜边的长度除以虚线长度，得到要分为多少段;
	...
	presenter.prototype.paint=function(opt){
		...
		for(var j=0;j<=this.connectors.length-1;j++){
			...
			_connector.begin.widget.connections.push(_connector);
			_connector.end.widget.connections.push(_connector);
...
//			return;
		...
		let data=[];
		for(var i=0;i<optionList.length;i++){
			if(optionList[i].name!=this.focuswidget.name){
				data.push(optionList[i])
			}
		}
			...
			if (this.focuswidget!=null&&this.focuswidget.editable){
				if(!this.is_mouse_down){
					...
					if(this.drawingconnection){
						...
						//控件功能点改变时发生
						// console.log(this.focuswidget,offsetX,offsetY,"------------------------------")
						if(this.focuswidget.afterfunctionPoint){
							var ev2={
									 name:this.focuswidget.name,
									  offset:{
										  x:offsetX,
										  y:offsetY
									  },

...
//			console.info(this.text+":"+this.focus);
			if(font!=null&&text){
	...
	table=function(option){
		...
		this.drawBorderTo=function(ctx){
			...
			$(this.fields).each(function(i,item){
				ths.findfield(item.name).y=(i+1+pkcount)*(ths.lineheight)+1;
			});
			var _widget=this;
			if (_widget.border.color !== "none") {
				...
				ctx.beginPath();
				ctx.moveTo(0,(pkcount+1)*(ths.lineheight));
				ctx.lineTo(this.width,(pkcount+1)*(ths.lineheight));
				ctx.stroke();
				ctx.restore();
			}
		...
		this.fillBackgroundTo=function(ctx){
			if (this.background!=null && this.background.color !== "none") {
				...
				ctx.fillRect(0, ths.lineheight, this.width, pkcount*(ths.lineheight));
				ctx.fillRect(0, (pkcount+1)*(ths.lineheight)+1,this.width,this.height-(pkcount+1)*(ths.lineheight)-1);
				ctx.restore();				
			}
		...
		this.deletefield=function(field){
			...
			if(focusfield!=null&&focusfield.data.isfk){
				alert("外键不能这样删除,如果要删除外键，请删除连接关系");
				e.cancel=true;
				return;
			}
